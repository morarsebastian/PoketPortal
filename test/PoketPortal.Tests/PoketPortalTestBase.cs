...
namespace PoketPortal.Tests
{
    public abstract class PoketPortalTestBase : AbpIntegratedTestBase<PoketPortalTestModule>
    {
        ...
        protected void UsingDbContext(Action<PoketPortalDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PoketPortalDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PoketPortalDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PoketPortalDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PoketPortalDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PoketPortalDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PoketPortalDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PoketPortalDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PoketPortalDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PoketPortalDbContext, Task<T>> func)
        {
            T result;
